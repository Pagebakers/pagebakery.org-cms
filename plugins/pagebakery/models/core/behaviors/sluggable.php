...
class SluggableBehavior extends ModelBehavior
{
	...
	function beforeSave(&$Model)
	{
		...
		if ($Model->hasField($this->__settings[$Model->alias]['slug']) && ($this->__settings[$Model->alias]['overwrite'] || empty($Model->data[$Model->alias][$this->__settings[$Model->alias]['slug']])))
		{
			...
			// Check if custom slug allready excist..
			$slug = $this->__slug($Model->data[$Model->alias][$this->__settings[$Model->alias]['slug']], $this->__settings[$Model->alias]);
			...
			if($result){
				// Slug allready excist, set error message
				$Model -> invalidate('slug', __d('wm', 'Slug allready excist', true));
				return false;
			}else{
				$Model->data[$Model->alias][$this->__settings[$Model->alias]['slug']] = $slug;
			}
